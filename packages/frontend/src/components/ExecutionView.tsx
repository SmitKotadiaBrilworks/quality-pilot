...
import { CheckCircle2, XCircle, Clock, Loader2, Square } from "lucide-react";
...
export function ExecutionView({ execution, onCancel }: ExecutionViewProps) {
  if (!execution) {
    return (
      <Card>
        <CardHeader>
        ...
        <div className="space-y-2">
          ...
          <div className="space-y-2 max-h-96 overflow-y-auto">
            {execution.steps.map((step, index) => (
              <div key={step.id} className="border rounded-lg p-3 space-y-1">
                <div className="flex items-center gap-2">
                ...
                {step.assertion && (
                  <div className="text-sm ml-6">
                    ...
                    >
                      Assertion: {step.assertion.type} -{" "}
                      {step.assertion.passed ? "✓ Passed" : "✗ Failed"}
                    </span>
                  </div>
                )}
                {step.error && (
